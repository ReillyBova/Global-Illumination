...
// Useful struct for finding closest points quickly
template <class PtrType>
struct PointAndDistanceSqd {
   PtrType point;
   RNLength distance_squared;
   bool operator<(PointAndDistanceSqd const& that) const {
    return this->distance_squared < that.distance_squared;
  }
};
...
public:
  ...
  // Optimized point approach
  int FindClosestQuick(PtrType query_point, RNScalar min_distance, RNScalar max_distance, int max_points,
      vector<PointAndDistanceSqd<PtrType> >& nearby_points) const;
  int FindClosestQuick(PtrType query_point,
        RNScalar min_distance, RNScalar max_distance, int max_points,
        int (*IsCompatible)(PtrType, PtrType, void *), void *compatible_data,
        vector<PointAndDistanceSqd<PtrType> >& nearby_points) const;
  int FindClosestQuick(const R3Point& query_position, RNScalar min_distance, RNScalar max_distance, int max_points,
          vector<PointAndDistanceSqd<PtrType> >& nearby_points) const;
...
public:
  ...
  PtrType FindAny(R3KdtreeNode<PtrType> *node, const R3Box& node_box,
    ...
    RNLength min_distance_squared, RNLength max_distance_squared,
    int (*IsCompatible)(PtrType, PtrType, void *), void *compatible_data) const;
  void FindClosestQuick(R3KdtreeNode<PtrType> *node, const R3Box& node_box,
      PtrType query_point, const R3Point& query_position,
